...
for normalization in [True, False]:
    ...
    for alpha in [0.0001, 0.001, 0.01, 0.1, 1]:
        ...
        for slr in [0.0001, 0.001, 0.01, 0.1]:
            
            # run over seeds
            for seed in range(10):
                
                if os.path.isfile(f'./result/super pNN/model/spnn_{normalization}_{alpha}_{slr}_{seed}'):
                    
                    print(f'./result/super pNN/model/spnn_{normalization}_{alpha}_{slr}_{seed} exists, skip this training.')
                
                else:
                
                    print(f'Training with normalization:{normalization} alpha:{alpha} lr:{slr} seed:{seed}.')
                    ...
                    torch.save(SuperPNN, f'./result/super pNN/model/spnn_{normalization}_{alpha}_{slr}_{seed}')