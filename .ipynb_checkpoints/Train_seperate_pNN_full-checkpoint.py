...
for dataset in datasets:
    ...
    for seed in range(10):
        ...
        X_train = config.DataReshape(X_train, config.MAX_IN)
        X_valid = config.DataReshape(X_valid, config.MAX_IN)
        X_test = config.DataReshape(X_test, config.MAX_IN)
        ...
        for l in range(len(config.full_topology)-1):
            pNN.add_module(f'pLayer {l}', pnn.pLayer(config.full_topology[l], config.full_topology[l+1]))
        ...
        pNN, train_losses, valid_losses, _, _ = training.train_nn(pNN,
                                                ...
                                                optimizer, pnn.LossFunction, device, UUID='seperatepNNfull')
