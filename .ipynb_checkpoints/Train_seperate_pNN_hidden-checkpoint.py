...
#SBATCH --job-name=SeperatePnnHidden
...
for t in config.hidden_topology:
    topology_name += str(t)
whole_file_path = f'./result/seperate pNN {topology_name} hidden'
...
for dataset in datasets:
    ...
    for seed in range(10):
        ...
        pNN.add_module(f'input pLayer', pnn.pLayer(N_feature, config.hidden_topology[0]))
        for l in range(len(config.hidden_topology)-1):
            pNN.add_module(f'pLayer {l}', pnn.pLayer(config.hidden_topology[l], config.hidden_topology[l+1]))
        pNN.add_module(f'output pLayer', pnn.pLayer(config.hidden_topology[-1], N_class))
        ...
        pNN, train_losses, valid_losses, _, _ = training.train_nn(pNN,
                                                ...
                                                optimizer, pnn.LossFunction, device, UUID='seperatepNN')
